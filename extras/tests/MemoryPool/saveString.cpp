...
static const char *saveString(MemoryPool &pool, const std::string &s) {
...
TEST_CASE("MemoryPool::saveString()") {
  ...
  SECTION("Deduplicates identical strings that contain NUL") {
    const char *a = saveString(pool, std::string("hello\0world", 11));
    const char *b = saveString(pool, std::string("hello\0world", 11));
    REQUIRE(a == b);
